...
test.describe('remote configuration', () => {
  createTest('should be fetched and applied')
    ...
    .run(async ({ page }) => {
      ...
      const initConfiguration = await page.evaluate(() => window.MD_RUM!.getInitConfiguration()!)
  ...
  createTest('should resolve user context')
    ...
    .run(async ({ page }) => {
      await waitForRemoteConfigurationToBeApplied(page)
      const user = await page.evaluate(() => window.MD_RUM!.getUser())
  ...
  createTest('should resolve global context')
    ...
    .run(async ({ page }) => {
      await waitForRemoteConfigurationToBeApplied(page)
      const globalContext = await page.evaluate(() => window.MD_RUM!.getGlobalContext())
