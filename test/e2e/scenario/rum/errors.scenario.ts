import type { RumErrorEvent } from '@motadata365/browser-rum-core'
...
test.describe('rum errors', () => {
  ...
  createTest('send runtime errors happening before initialization')
    ...
    .withRumInit((configuration) => {
      ...
      setTimeout(() => window.MD_RUM!.init(configuration))
    })
  ...
  createTest('send custom errors')
    .withRum()
    .withBody(createBody('MD_RUM.addError(foo())'))
    .run(async ({ flushEvents, page, intakeRegistry, baseUrl, withBrowserLogs }) => {
      const button = page.locator('button')
      await button.click()
  ...
  createTest('send non-native errors')
    .withRum()
    .withBody(createBody('MD_RUM.addError(customError())'))
    .run(async ({ flushEvents, page, intakeRegistry, baseUrl, withBrowserLogs }) => {
      const button = page.locator('button')
      await button.click()
  ...
  createTest('send non-native errors with inheritance')
    .withRum()
    .withBody(createBody('MD_RUM.addError(customErrorWithInheritance())'))
    .run(async ({ flushEvents, page, intakeRegistry, baseUrl, withBrowserLogs }) => {
      const button = page.locator('button')
      await button.click()
