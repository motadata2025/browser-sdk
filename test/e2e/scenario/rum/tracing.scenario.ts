...
test.describe('tracing', () => {
  ...
  createTest('propagate trace baggage')
    ...
    .run(async ({ intakeRegistry, flushEvents, page }) => {
      await page.evaluate(() => {
        window.MD_RUM!.setUser({ id: 'p1745' })
        window.MD_RUM!.setAccount({ id: 'c9wpq8xrvd9t' })
      })
      const rawHeaders = await page.evaluate(() =>
        window
          .fetch('/headers')
  ...
  function checkRequestHeaders(
    ...
    expect(headers['x-motadata-trace-id']).toMatch(/\d+/)
    expect(headers['x-motadata-origin']).toBe('rum')
  ...
  function checkTraceAssociatedToRumEvent(intakeRegistry: IntakeRegistry) {
    ...
    expect(requests[0]._md.trace_id).toMatch(/\d+/)
    expect(requests[0]._md.span_id).toMatch(/\d+/)
