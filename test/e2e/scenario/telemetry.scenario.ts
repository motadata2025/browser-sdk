...
test.describe('telemetry', () => {
  createTest('send errors for logs')
    ...
    .run(async ({ intakeRegistry, page, flushEvents }) => {
      await page.evaluate(() => {
        ...
        window.MD_LOGS!.logger.log('hop', context as any)
      })
      await flushEvents()
  ...
  createTest('send errors for RUM')
    ...
    .run(async ({ intakeRegistry, flushEvents, page }) => {
      await page.evaluate(() => {
        ...
        window.MD_RUM!.addAction('hop', context as any)
      })
      await flushEvents()
      expect(intakeRegistry.telemetryErrorEvents).toHaveLength(1)
      const event = intakeRegistry.telemetryErrorEvents[0]
  ...
  createTest('send usage telemetry for logs')
    ...
    .run(async ({ intakeRegistry, flushEvents, page }) => {
      await page.evaluate(() => {
        window.MD_LOGS!.setTrackingConsent('granted')
      })
  ...
  createTest('stops sending telemetry after consent revocation')
    ...
    .run(async ({ intakeRegistry, flushEvents, page }) => {
      ...
      await page.evaluate(() => {
        window.MD_RUM!.addAction('initial-action')
        window.MD_RUM!.setTrackingConsent('not-granted')
        window.MD_RUM!.addAction('post-revocation-action')
        window.MD_RUM!.getAccount()
      })
