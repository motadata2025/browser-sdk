import type { RumEvent, RumEventDomainContext, RumInitConfiguration } from '@motadata365/browser-rum-core'
import type { LogsEvent, LogsInitConfiguration, LogsEventDomainContext } from '@motadata365/browser-logs'
...
test.describe('microfrontend', () => {
  ...
  createTest('expose handling stack for MD_RUM.addAction')
    .withRum(RUM_CONFIG)
    .withRumInit((configuration) => {
      window.MD_RUM!.init(configuration)
  ...
  createTest('expose handling stack for MD_RUM.addError')
    .withRum(RUM_CONFIG)
    .withRumInit((configuration) => {
      window.MD_RUM!.init(configuration)

      function testHandlingStack() {
        window.MD_RUM!.addError(new Error('foo'))
      }
  ...
  test.describe('logger apis', () => {
    createTest('expose handling stack for MD_LOGS.logger.log')
      .withLogs(LOGS_CONFIG)
      .withLogsInit((configuration) => {
        window.MD_LOGS!.init(configuration)
