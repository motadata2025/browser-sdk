...
export function mockSyntheticsWorkerValues(
  ...
  switch (method) {
    case 'globals':
      ;(window as BrowserWindow)._MOTADATA_SYNTHETICS_PUBLIC_ID = publicId
      ;(window as BrowserWindow)._MOTADATA_SYNTHETICS_RESULT_ID = resultId
      ;(window as BrowserWindow)._MOTADATA_SYNTHETICS_INJECTS_RUM = injectsRum
      break
    case 'cookies':
  ...
  registerCleanupTask(() => {
    delete (window as BrowserWindow)._MOTADATA_SYNTHETICS_PUBLIC_ID
    delete (window as BrowserWindow)._MOTADATA_SYNTHETICS_RESULT_ID
    delete (window as BrowserWindow)._MOTADATA_SYNTHETICS_INJECTS_RUM
